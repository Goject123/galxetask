...
const RegisterPage: React.FC<RegisterPageProps> = ({ onRegister, onNavigateToLogin }) => {
  ...
  const handleGoogleRegister = () => {
    // 模拟 Google 注册
    onRegister('user@gmail.com');
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (email && verificationCode && agreeTerms) {
      onRegister(email);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">加入 Alphaping</h1>
          <p className="text-gray-600">创建您的账户，开始智能任务监控之旅</p>
        </div>

        {/* Register Form */}
        <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">创建账户</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <label htmlFor="terms" className="text-sm text-gray-600">
                我同意 <span className="text-blue-600 hover:text-blue-800 cursor-pointer">服务条款</span> 和 <span className="text-blue-600 hover:text-blue-800 cursor-pointer">隐私政策</span>
              </label>
            </div>
            ...
            <button
              type="submit"
              disabled={!email || !verificationCode || !agreeTerms}
